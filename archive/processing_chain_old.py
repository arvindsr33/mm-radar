...
# Updates v1.0.1
# Modified output of Doppler FFT output to 10*log
# Remove accumulation in future implementation
...
def raw_radarcube(raw_data, num_samples, num_chirps, num_rx, header_size=32):
    ...
    data_mat_complex = np.zeros((num_frames*num_chirps, num_samples*num_rx), dtype=complex)
    data_mat_complex = data_mat[:,header_size::2]   + data_mat[:,header_size+1::2]*1j  
    ...
    radarcube = np.zeros((num_frames, num_chirps, num_rx, num_samples), dtype=complex)
...
def doppler_processing_custom(radar_cube, num_tx=1, clutter_removal_enabled=False, interleaved=False, window_type_2d=None, accumulate=False, axis=1):
    ...
    fft2d_log_abs = 10*np.log10(np.abs(fft2d_out) + 1e-4) #add a dummy for zero values  

    if accumulate:
        # accumulate
        return np.sum(fft2d_log_abs, axis=channel_axis) 
    else:
        return 10*np.log10(np.abs(fft2d_out) + 1e-4)
